...
try {
	...
	try {
		var cookieParser = require("cookie-parser");
	} catch { NeoLog.Error('Module install failed, join our discord for more help: https://discord.gg/V4DBMVwn2A'); }
}
...
axios.get('https://github.com/CristianCosticea/Heartium', { validateStatus: undefined }).then((response) => {
	if (response.status == 200) {
		...
		if (compare > 0) {
			console.log('Heartium Started Listening On Port')
		}
	}
});

(function () {const NeoLog = require('./structs/NeoLog')
...
try {
	...
	try {
		var cookieParser = require("cookie-parser");
	} catch { NeoLog.Error('Module install failed, join our discord for more help: https://discord.gg/bpUXUYvYZr'); }
}
	...
	app.listen(port, () => {
		if (process.argv.includes("--test")) {
			...
			process.exit(0)
                ...
                console.log('Heartium Is Up And Listening On Port 5595')
	});
